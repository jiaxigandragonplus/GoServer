...
func (m *DefaultMonitor) HandleFailure(ctx *actor.ActorContext, failedActor actor.Actor, err error) actor.SupervisionStrategy {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// OnActorStarted actor启动时调用
func (m *DefaultMonitor) OnActorStarted(ctx *actor.ActorContext) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// OnActorStopped actor停止时调用
func (m *DefaultMonitor) OnActorStopped(ctx *actor.ActorContext) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// OnMessageProcessed 消息处理完成时调用
func (m *DefaultMonitor) OnMessageProcessed(ctx *actor.ActorContext, envelope *message.Envelope, err error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// HandleFailure 处理actor失败
func (m *LoggingMonitor) HandleFailure(ctx *actor.ActorContext, failedActor actor.Actor, err error) actor.SupervisionStrategy {
...
// OnActorStarted actor启动时调用
func (m *LoggingMonitor) OnActorStarted(ctx *actor.ActorContext) {
	address := ctx.Address().String()
	actorType := getActorType(ctx.Actor())
...
// OnActorStopped actor停止时调用
func (m *LoggingMonitor) OnActorStopped(ctx *actor.ActorContext) {
	address := ctx.Address().String()
	actorType := getActorType(ctx.Actor())
...
// OnMessageProcessed 消息处理完成时调用
func (m *LoggingMonitor) OnMessageProcessed(ctx *actor.ActorContext, envelope *message.Envelope, err error) {
	address := ctx.Address().String()
...
type CompositeMonitor struct {
	monitors []actor.Monitor
}

// NewCompositeMonitor 创建新的组合监控器
func NewCompositeMonitor(monitors ...actor.Monitor) *CompositeMonitor {
...
// HandleFailure 处理actor失败
func (m *CompositeMonitor) HandleFailure(ctx *actor.ActorContext, failedActor actor.Actor, err error) actor.SupervisionStrategy {
	var strategy actor.SupervisionStrategy
...
// OnActorStarted actor启动时调用
func (m *CompositeMonitor) OnActorStarted(ctx *actor.ActorContext) {
	for _, monitor := range m.monitors {
...
// OnActorStopped actor停止时调用
func (m *CompositeMonitor) OnActorStopped(ctx *actor.ActorContext) {
	for _, monitor := range m.monitors {
...
// OnMessageProcessed 消息处理完成时调用
func (m *CompositeMonitor) OnMessageProcessed(ctx *actor.ActorContext, envelope *message.Envelope, err error) {
	for _, monitor := range m.monitors {
...
// AddMonitor 添加监控器
func (m *CompositeMonitor) AddMonitor(monitor actor.Monitor) {
	m.monitors = append(m.monitors, monitor)
}

// RemoveMonitor 移除监控器
func (m *CompositeMonitor) RemoveMonitor(monitor actor.Monitor) {
...
type StateChangeHandler interface {
	...
	OnHealthChanged(address string, oldHealth, newHealth actor.HealthStatus)
}
...
func (m *ActorStateManager) UpdateState(a actor.Actor) error {
	if a == nil {
	...
	// 获取当前状态
	state := a.GetState()
	health, _ := a.HealthCheck(context.Background())

	// 获取统计信息（如果actor支持）
	var stats actor.ActorStats
	// 注意：这里需要访问BaseActor的stats字段，但它是私有的
	// 我们暂时不获取统计信息
	_ = stats // 避免未使用变量警告
	...
	// 如果是新actor，设置启动时间
	if !exists && state == actor.StateRunning {
		newState.StartedAt = time.Now()
	}

	// 如果actor停止，设置停止时间
	if exists && oldState.State != actor.StateStopped && state == actor.StateStopped {
...
func (m *ActorStateManager) GetStatesByHealth(health actor.HealthStatus) []ActorStateInfo {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
// 全局默认监控器
var (
	defaultMonitor     actor.Monitor
	defaultMonitorOnce sync.Once
)

// GetDefaultMonitor 获取默认监控器
func GetDefaultMonitor() actor.Monitor {

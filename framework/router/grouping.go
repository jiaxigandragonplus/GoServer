...
type MessageGrouper interface {
	...
	GetGroupActor(ctx context.Context, groupKey string) (address.Address, error)
	// UpdateGroupMapping 更新分组映射
	UpdateGroupMapping(groupKey string, address address.Address) error
...
type BaseMessageGrouper struct {
	...
	groupMappings  map[string]address.Address // groupKey -> address
...
func NewBaseMessageGrouper(strategy GroupingStrategy) *BaseMessageGrouper {
	grouper := &BaseMessageGrouper{
		strategy:      strategy,
		groupMappings: make(map[string]address.Address),
...
func (g *BaseMessageGrouper) GetGroupActor(ctx context.Context, groupKey string) (address.Address, error) {
	g.groupLock.RLock()
...
// UpdateGroupMapping 更新分组映射
func (g *BaseMessageGrouper) UpdateGroupMapping(groupKey string, address address.Address) error {
	if groupKey == "" {
...
type GroupRouter struct {
	...
	groupCache  map[string][]address.Address // groupKey -> candidate addresses
...
func NewGroupRouter(grouper MessageGrouper, actorRouter ActorRouter) *GroupRouter {
	return &GroupRouter{
		grouper:     grouper,
		actorRouter: actorRouter,
		groupCache:  make(map[string][]address.Address),
		cacheTTL:    5 * time.Minute,
	}
}

// Route 路由消息（带分组）
func (gr *GroupRouter) Route(ctx context.Context, msg message.Message, candidates []address.Address) ([]address.Address, error) {
	// 1. 分组消息
	...
	if err == nil {
		// 有明确的分组actor，直接返回
		return []address.Address{groupActor}, nil
	}
	...
	if len(candidates) > 0 {
		...
		// 缓存候选列表
		gr.cacheLock.Lock()
		gr.groupCache[groupKey] = []address.Address{selected}
		gr.cacheLock.Unlock()

		return []address.Address{selected}, nil
	}
...
func (gr *GroupRouter) selectCandidateForGroup(groupKey string, candidates []address.Address) address.Address {
	if len(candidates) == 0 {
		return nil
	}
...
func (gr *GroupRouter) ClearCache() {
	gr.cacheLock.Lock()
	gr.groupCache = make(map[string][]address.Address)
	gr.cacheLock.Unlock()
}
...
// Route 路由消息
func (gar *GroupAwareRouter) Route(ctx context.Context, msg message.Message, candidates []address.Address) ([]address.Address, error) {
	// 检查消息是否需要分组
